...
from interceptor_control.missions.transition.transition_logger import TransitionLogger

class Transition1Mission(Node):
    def __init__(self):
        super().__init__('transition_1_mission')
        ...
        self.logger = TransitionLogger(workspace_path)
        ...
        self.get_logger().info("Transition 1 Mission Node Started")
    ...
    def timer_callback(self):
        ...
        if self.state != "END":
            ...
            if self.state in [
                ...
                elif self.state == "FORWARD_FLIGHT":
                    commanded_vx = self.target_speed

                elif self.state == "TRANSITION_IN":
                    elapsed = (
                ...
                self.commanded_vx = commanded_vx
                self.commanded_vy = commanded_vy
                self.commanded_vz = commanded_vz
        ...
        if self.state == "INIT":
            ...
            if dist < self.acceptance_radius:
                self.get_logger().info(
                    "Takeoff complete. Holding hover..."
                )
            ...
            self.get_logger().info(
                f"Hovering before transition: "
                f"{hover_elapsed:.1f}s / 2.0s"
            )

            if hover_elapsed >= 2.0:
                self.get_logger().info(
                    "Hover complete. Starting transition out..."
                )
