...
class OffboardHover(Node):
    ...
    def _control_loop(self):
        ...
        if self.state == FlightState.IDLE:
            self.publish_setpoint(z=self.target_alt)
            ...
            if self.setpoint_counter >= 10:  # 1초 (10Hz × 10)
            ...
            if alt_err < self.pos_threshold:
                ...
                self.get_logger().info(
                    f'→ HOVER (z={self.local_pos.z:.2f}m, {self.hover_duration:.0f}s 시작)')
            ...
            if elapsed >= self.hover_duration:
                self._print_hover_stats()
                self.state = FlightState.LANDING
                self.get_logger().info('→ LANDING (호버 완료)')
    ...
    def _print_hover_stats(self):
        ...
        if len(self.hover_data) > 0:
            data = np.array(self.hover_data)
            xs = data[:, 0]
            ys = data[:, 1]
            zs = data[:, 2]
            ...
            max_dev = np.max(np.sqrt(
                (xs - self.takeoff_xy[0])**2 +
                (ys - self.takeoff_xy[1])**2
            ))
        else:
            std_x = 0.0
            std_y = 0.0
        ...
        self.get_logger().info(f'  x 표준편차    : {std_x:.3f}m')
        self.get_logger().info(f'  y 표준편차    : {std_y:.3f}m')
        self.get_logger().info(f'  z 표준편차    : {std_z:.3f}m')
        self.get_logger().info(f'  수평 std      : {std_xy:.3f}m')
        self.get_logger().info(f'  최대 편차     : {max_dev:.3f}m')
