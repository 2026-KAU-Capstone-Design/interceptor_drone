...
class OffboardHover(Node):
    def __init__(self):
        ...
        # Mission 3: 10m x 10m square waypoints
    ...
    def _control_loop(self):
        ...
        if self.state == FlightState.IDLE:
            ...
            # A 기준 10m x 10m 사각형 waypoint
            self.waypoints = [
                (ax + 10.0, ay + 0.0, self.target_alt),   # B
                (ax + 10.0, ay + 10.0, self.target_alt),  # C
                (ax + 0.0,  ay + 10.0, self.target_alt),  # D
                (ax + 0.0,  ay + 0.0,  self.target_alt),  # A return
            ]
            ...
            if alt_err < self.pos_threshold:
                ...
                self.get_logger().info('→ NAVIGATE: square flight start')
            ...
            self.publish_setpoint(x=target_x, y=target_y, z=target_z)

            dist = horizontal_distance_to(target_x, target_y)

            if dist < self.pos_threshold:
                self.get_logger().info(
                    f'Waypoint {self.current_wp_idx} 도달: '
                    f'target=({target_x:.1f}, {target_y:.1f}), error={dist:.3f}m'
                )
                ...
                if self.current_wp_idx >= len(self.waypoints):
                    self.state = FlightState.LANDING
                    self.get_logger().info('→ LANDING: square flight complete')
                else:
                    next_wp = self.waypoints[self.current_wp_idx]
                    self.get_logger().info(
                        f'→ NEXT WP {self.current_wp_idx}: '
