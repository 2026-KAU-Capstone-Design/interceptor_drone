...
| 표적 검출 | YOLOv8n (Ultralytics) + PyTorch |
...
### Perception 환경 추가 설치 (YOLOv8n)

기본 환경 설치 후, 표적 검출을 위한 YOLOv8n 환경을 추가로 설치합니다.

```bash
bash scripts/setup_perception.sh
```

- **NVIDIA GPU** 가 있으면 자동으로 CUDA PyTorch 설치 (GPU 추론)
- **NVIDIA GPU 가 없으면** (AMD, Intel iGPU 등) 자동으로 CPU PyTorch 설치 (CPU 추론)
- GPU 종류에 관계없이 **같은 스크립트, 같은 코드, 같은 결과**. 속도만 차이남.
- 설치 완료 후 자동 검증 (PyTorch 버전, CUDA 여부, YOLOv8n 추론 테스트).

설치가 끝나면 동작 확인:

```bash
yolo predict model=yolov8n.pt source="https://ultralytics.com/images/bus.jpg" save=True
xdg-open runs/detect/predict/bus.jpg
```

결과 이미지에 사람, 버스 등이 bounding box로 검출되면 성공.
...
T3에서 `/fmu/out/vehicle_status_v1` 같은 토픽이 보이면 환경 구성 성공.
(PX4 v1.16에서 `vehicle_status` → `vehicle_status_v1` 로 이름 변경됨)
...
├── scripts/
│   ├── setup_dev_env.sh    ← 기본 환경 자동 설치 (ROS2+PX4+Gazebo, 멱등)
│   ├── setup_perception.sh ← Perception 환경 설치 (YOLOv8n+PyTorch)
│   └── verify_env.sh       ← 환경 검증 (출력 비교용)
├── requirements/
│   └── perception.txt      ← Python 의존성 참고 목록
├── repos/
